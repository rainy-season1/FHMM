...
from nilmtk.losses import f1score
...
def draw(dic,appliances_name,pre_data,gt_data):
    ...
    for i in range(n):
        ...
        print('MAE:',mean_absolute_error(pre_data[:,i],gt_data[i][:,:1]))
...
physical_quantity=['P','Q','I','Pt','DPF']#所用电器物理量
cluster_list=[2,2,6,5,4,3]#各电器状态个数
...
all_transform_data,gt_data=fhmm_test_data(data_set,appliances_name,['P','Q'],
                                  scaler_list,train_min=20000,train_max=30000)
#状态预测
...
draw(appliance_dic,appliances_name,pre_data,gt_data)
